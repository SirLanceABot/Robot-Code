#include "ITG-3200.h"
#include "WPILib.h" // for I2C, Notifier, and Timer
#include "Constants.h" // not generally needed; only for Test function
#include "XboxJoystick.h" // not generally needed; only for Test function
...
m_backgroundLoop (std::make_unique<Notifier>(&ITG3200::Get, this)), // using WPILib Notifier for periodic task
mTimer (new Timer()),
		...
		while(xbox->GetX() == false)
		{
			...
			while(xbox->GetA())
			{
				...
				if(leftY != 0 && xbox->GetA())
				{
					GetAngle(angle);
					GetAngleRate(angleRate);
					printf("[ITG3200] temperature: %7.2f; X: %9.2f, Y: %9.2f, Z: %9.2f; X rate: %9.2f, Y rate: %9.2f, Z rate: %9.2f\n",
							GetTemperature(),
