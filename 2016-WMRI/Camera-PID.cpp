...
int TargetMain(CANTalon * mMotorA, CANTalon * mMotorB, CANTalon * mMotorC, CANTalon * mMotorD)
{
   ...
   \\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//
	\/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/ */
		...
		double imageWidth = roboRealm->GetNumber("IMAGE_WIDTH", 320);
		double xPosition;
		...
		input = imageWidth/2.L + 6.0 - xPosition; // positive deviation means object too far left.  positive power makes deviation more positive-direct control

		if(abs(input) <= 4) // pick so robot will coast to the center - no more, no less
		{
   ...
   \\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//
	...
	mMotorC->SetControlMode(mMotorC->ControlMode::kFollower);
	mMotorC->Set(CAN_PORT::FRONT_LEFT);

	mMotorD->SetControlMode(mMotorD->ControlMode::kFollower);
	mMotorD->Set(CAN_PORT::FRONT_RIGHT);
