...
{
	// changes shooter elevation - up or down as needed
	// returns true if not at shooting angle - motor still running
	// returns false if at shooting angle - motor off
	bool done = true; // badly named variable
	int shooterAngle = abs(mRaiseTalon->GetEncPosition());
	printf("Shooter Enc: %d\n", shooterAngle);
	...
	else if(shooterAngle < AUTONOMOUS::SHOOTER_ANGLE_VISION)
		mRaiseTalon->Set(-.44);
	else if(shooterAngle > AUTONOMOUS::SHOOTER_ANGLE_VISION)
		mRaiseTalon->Set(.44);
...
bool Shooter::RaiseToJamShotAngle()
{
	...
	else if(shooterAngle < AUTONOMOUS::SHOOTER_ANGLE_JAM_SHOT)
		mRaiseTalon->Set(-.37);
	else if(shooterAngle > AUTONOMOUS::SHOOTER_ANGLE_JAM_SHOT)
		mRaiseTalon->Set(.37);
